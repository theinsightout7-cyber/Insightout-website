    ...
    <div className='mt-24 px-4 md:px-8 lg:px-16 container mx-auto flex flex-col'>
      ...
      <div className='max-w-4xl mx-auto space-y-6'>
        {Array.isArray(blog.contentBlocks) &&
          blog.contentBlocks.map((blockRaw: unknown, index: number) => {
            ...
            const filteredItems = subtitleTitle
              ? items.filter((it) => it !== subtitleTitle)
              : items
            ...
            return (
              <div key={index}>
                ...
                {block.subtitles?.description && (
                  <p>{block.subtitles?.description}</p>
                )}
                ...
                {filteredItems.length === 1 ? (
                  ...
                  <ul className='list-disc list-outside mt-2 text-gray-700 leading-relaxed space-y-1 pl-5'>
                    {filteredItems.map((item, i) => (
                      <li
                        key={i}
                        className='marker:text-gray-600 whitespace-pre-line '
                      >
                        <p>{item}</p>
                      </li>
                    ))}
